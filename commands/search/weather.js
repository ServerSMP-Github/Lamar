    ...
    message.reply({
      embeds: [
        new EmbedBuilder()
          ...
          .addField("Celsius - KPH", `Current Temp 🌡️ - ${response.current.temp_c}°C\nFeelslike🖐 - ${response.current.feelslike_c}°C\nCondition 🌦️ - ${response.current.condition.text}`, true)
          .addField("Fahrenheit - MPH", `Current Temp 🌡️ - ${response.current.temp_f}°F\nFeelslike 🖐 - ${response.current.feelslike_f}°F\nCondition 🌦️ - ${response.current.condition.text}`, true)
          .setColor("Random")
      ]
    });
  }
